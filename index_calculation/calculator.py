...
def _calculate_RQS(asset_table: AssetTable) -> Decimal:
...
def calculate_FRRS(coin_data: CoinData) -> Index:
    ...
    TA_score :Decimal = 1.00 if coin_data.asset_table.cusip_appearance else 0.85 # Transparency Adjustment Score
    collateralization_ratio :Decimal = coin_data.asset_table.total.amount / coin_data.onchain_data.outstanding_token
    SA_score :Decimal = 1.0 + 0.05 * log(collateralization_ratio - 1.0) * 100 + 1 if collateralization_ratio > 1 else 0
